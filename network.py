...
class Network:
    def __init__(self):
        self.images, self.labels = loadlocal_mnist(
            images_path=r"C:\Users\andka\Desktop\training_data\train-images.idx3-ubyte",
            labels_path=r"C:\Users\andka\Desktop\training_data\train-labels.idx1-ubyte"
        )
    ...
    def back_prop(self, start, stop):
        ...
        self.weights = [w + (adj/(num_of_inputs*144)) for w, adj in list(zip(self.weights, weight_adjustments))]
        self.biases = [b + (adj/(num_of_inputs*144)) for b, adj in list(zip(self.biases, biases_adjustments))]
        #print('Now testing')
    ...
    def test_input(self):
        print(self.labels[0])
        a, _, _ = self.compute_output(self.images[0])
        print(a)
